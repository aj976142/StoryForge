...
class VoiceInputController(context: Context) {
    ...
    private val _state = MutableStateFlow(
        VoiceState(available = SpeechRecognizer.isRecognitionAvailable(appContext))
    )
    ...
    fun start(onFinal: (String) -> Unit) {
        if (!SpeechRecognizer.isRecognitionAvailable(appContext)) {
            _state.value = VoiceState(
                available = false,
                error = "Voice input isn't available on this device. Type your idea instead."
            )
            return
        }
        stop()
        ...
        speech.setRecognitionListener(object : RecognitionListener {
            override fun onReadyForSpeech(params: Bundle?) {
                _state.value = VoiceState(available = true, listening = true)
            }
            ...
            override fun onResults(results: Bundle?) {
                val text = results
                    ...
                    ?.firstOrNull()
                    .orEmpty()
                _state.value = VoiceState(available = true, listening = false, partial = text)
    ...
    private fun humanError(code: Int): String = when (code) {
        ...
        SpeechRecognizer.ERROR_NO_MATCH ->
            "I didn't catch that. Try again or type the idea."
        SpeechRecognizer.ERROR_SPEECH_TIMEOUT ->
            "No speech detected. Tap the mic and speak."
        SpeechRecognizer.ERROR_NETWORK, SpeechRecognizer.ERROR_NETWORK_TIMEOUT ->
            "Voice recognition needs a network connection on this device."
