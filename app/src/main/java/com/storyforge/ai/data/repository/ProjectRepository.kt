...
class ProjectRepository(private val store: ProjectStore) {
    ...
    suspend fun save(project: Project): Project = mutationMutex.withLock {
        ...
        val trusted = manuscript.isNotBlank() && current.generatedForIdeaHash.isNotBlank() && current.generatedForIdeaHash == GenerationProvenance.fingerprint(current.rawIdea, current.format)
        store.upsert(project.copy(revision = current.revision + 1, rawIdea = current.rawIdea, format = current.format, inputMode = current.inputMode, createdAt = current.createdAt, generatedForIdeaHash = if (trusted) current.generatedForIdeaHash else "", status = if (manuscript.isBlank()) ProjectStatus.DRAFT else ProjectStatus.SAVED, title = project.title.ifBlank { "Untitled" }, storyBible = current.storyBible, versions = appendVersion(current, manuscript, project.title, "Saved")))
    }
    ...
    suspend fun updateManuscript(id: String, text: String, title: String): Project? = mutationMutex.withLock {
        ...
        store.upsert(current.copy(generatedText = manuscript, generatedForIdeaHash = fingerprint, title = title.ifBlank { current.title }, status = if (manuscript.isBlank()) ProjectStatus.DRAFT else ProjectStatus.SAVED, revision = current.revision + 1, versions = appendVersion(current, manuscript, title, "Autosave")))
    }
    ...
    suspend fun restoreVersion(id: String, versionId: String): Project? = mutationMutex.withLock {
        ...
        store.upsert(current.copy(generatedText = manuscript, title = version.title.ifBlank { current.title }, generatedForIdeaHash = if (manuscript.isBlank()) "" else GenerationProvenance.fingerprint(current.rawIdea, current.format), status = if (manuscript.isBlank()) ProjectStatus.DRAFT else ProjectStatus.SAVED, revision = current.revision + 1, versions = appendVersion(current, current.generatedText, current.title, "Before restore")))
    }

    suspend fun updateStoryBible(id: String, bible: StoryBible): Project? = mutationMutex.withLock {
        val current = store.getProject(id) ?: return@withLock null
        if (deletedIds.contains(id)) return@withLock null
        store.upsert(current.copy(storyBible = bible, revision = current.revision + 1))
    }
