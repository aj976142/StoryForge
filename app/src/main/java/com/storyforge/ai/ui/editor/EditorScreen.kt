...
fun EditorScreen(
        ...
        when {
            ...
            state.project == null -> Column(Modifier.padding(16.dp)) { ErrorBanner(state.error ?: "Missing project", onDismiss = onBack) }
                ...
                BasicTextField(
                    ...
                    singleLine = true,
                    textStyle = TextStyle(color = MaterialTheme.colorScheme.onBackground, fontSize = 25.sp, lineHeight = 31.sp, fontWeight = FontWeight.SemiBold),
                    modifier = Modifier.fillMaxWidth(),
                    decorationBox = { inner -> if (state.title.isBlank()) Text("Untitled story", color = MaterialTheme.colorScheme.onSurfaceVariant, fontSize = 25.sp); inner() }
                )
                ...
                Box(Modifier.fillMaxWidth().weight(1f).verticalScroll(scrollState).padding(vertical = 8.dp)) {
                    BasicTextField(
                        ...
                        textStyle = TextStyle(color = MaterialTheme.colorScheme.onBackground, fontSize = 17.sp, lineHeight = 27.sp),
                        modifier = Modifier.fillMaxWidth(),
                        decorationBox = { inner -> if (state.text.isBlank()) Text("Start writing…", color = MaterialTheme.colorScheme.onSurfaceVariant, fontSize = 17.sp); inner() }
                    )
                }
    ...
    if (showRename) {
        AlertDialog(
            ...
            confirmButton = { TextButton(onClick = { viewModel.onTitleChange(renameText.trim()); viewModel.save(); showRename = false }) { Text("Save") } },
        ...
        AlertDialog(
            ...
            text = {
                Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
                    ...
                    Text("Estimated reading time: ${((wordCount + 179) / 180).coerceAtLeast(1)} min")
                    Text("Format: ${state.project?.format?.displayName ?: "Draft"}")
                }
            },
    ...
    if (showBrain) {
        ...
        AlertDialog(
            onDismissRequest = { showBrain = false },
            title = { Text("Story Brain") },
            text = {
                Column(verticalArrangement = Arrangement.spacedBy(10.dp)) {
                    Text("Continuity snapshot", style = MaterialTheme.typography.labelLarge, color = MaterialTheme.colorScheme.primary)
                    Text("Characters: ${brain.characters.ifEmpty { listOf("Not detected yet") }.joinToString(", ")}")
                    Text("Places: ${brain.places.ifEmpty { listOf("Not detected yet") }.joinToString(", ")}")
                    Text("Themes: ${brain.themes.ifEmpty { listOf("Not enough text yet") }.joinToString(", ")}")
                    Text("${brain.wordCount} words · ~${brain.readingMinutes} min read", color = MaterialTheme.colorScheme.onSurfaceVariant)
                    Text("This snapshot is generated offline on your device.", style = MaterialTheme.typography.bodySmall, color = MaterialTheme.colorScheme.onSurfaceVariant)
                }
            },
            confirmButton = { TextButton(onClick = { showBrain = false }) { Text("Done") } }
        )
    }
}
