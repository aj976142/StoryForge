...
fun InputScreen(
    ...
    Column(Modifier.fillMaxSize()) {
        ...
        Column(
            ...
            Text(
                "Dump the raw thought. Structure comes next.",
            ...
            Row(
                ...
                Text(
                    "${state.words} words · ${state.characters} characters",
                    style = MaterialTheme.typography.bodyMedium,
                    color = MaterialTheme.colorScheme.onSurfaceVariant
                )
                Text(
                    state.savedHint ?: if (state.saving) "Saving…" else "",
                    style = MaterialTheme.typography.bodyMedium,
            ...
            Row(horizontalArrangement = Arrangement.spacedBy(10.dp), modifier = Modifier.fillMaxWidth()) {
                FilledTonalIconButton(
                    ...
                    Icon(
                        if (state.listening) Icons.Outlined.Stop else Icons.Outlined.Mic,
                        contentDescription = "Microphone"
                    )
                }
                FilledTonalIconButton(onClick = viewModel::clear) {
                    Icon(Icons.Outlined.Close, contentDescription = "Clear")
                }
                Button(
                    onClick = { viewModel.continueNext(onContinue) },
